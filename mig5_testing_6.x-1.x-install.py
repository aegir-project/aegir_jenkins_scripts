...
def fab_prepare_firewall():
        ...
        fabric.run("for source in 174.136.104.138 59.167.182.161; do iptables -I INPUT -s $source -p tcp --dport 22 -j ACCEPT; iptables -I INPUT -s $source -p tcp --dport 80 -j ACCEPT; done; iptables -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPT; iptables --policy INPUT DROP", pty=True)
...
def main():
        ...
        try:
                node = conn.deploy_node(name='aegir_mig5', image=preferred_image[0], size=preferred_size[0], deploy=msd)                                             
        except:
                e = sys.exc_info()[1]
                raise SystemError(e)
